...
- [ ] 5.1 Create `src/energizados/web/templates/` directory structure (`base.html`, `index.html`, `job_list.html`, `job_detail.html`, `components/`)
- [ ] 5.2 Implement `base.html` layout with HTMX CDN script and Bootstrap CSS
- [ ] 5.3 Implement `components/editor.html` (YAML textarea + file upload input)
- [ ] 5.4 Implement `components/validation.html` (error messages from `validate_dict`)
- [ ] 5.5 Implement `components/status_badge.html` (color-coded status badge)
- [ ] 5.6 Implement `index.html` main page (editor + job list container)
- [ ] 5.7 Implement `job_list.html` HTMX fragment (table of jobs with cancel/retry buttons)
- [ ] 5.8 Implement `job_detail.html` HTMX fragment (single job row with status)
- [ ] 5.9 Create FastAPI app in `src/energizados/web/app.py` (init, CORS middleware, static files)
- [ ] 5.10 Implement `GET /` route (render index.html)
- [ ] 5.11 Implement `POST /jobs` route (parse YAML, validate via `validate_dict`, check `custom_class` prefixes, enqueue via `JobStore.create_job()`)
- [ ] 5.12 Implement `_check_custom_class_prefixes()` helper (extract all `custom_class` paths, validate against `ALLOWED_PREFIXES`)
- [ ] 5.13 Implement `GET /jobs` route (render job_list.html HTMX fragment, auto-refresh every 2s)
- [ ] 5.14 Implement `GET /jobs/{id}` route (render job_detail.html or JSON)
- [ ] 5.15 Implement `POST /jobs/{id}/cancel` route (call `JobStore.cancel_job()`, return JSON status)
- [ ] 5.16 Implement `POST /jobs/{id}/retry` route (call `JobStore.retry_job()`, return new job_id)
- [ ] 5.17 Implement `GET /health` route (return JSON `{"ok": true}`)
- [ ] 5.18 Implement `GET /api/runs` route (proxy `RunManager.list_runs()` for Phase 2 preparation)
- [ ] 5.19 Write unit tests for all routes with TestClient (`tests/web/test_app.py`)
- [ ] 5.20 Write unit tests for `custom_class` prefix validation (`tests/web/test_app.py`)
