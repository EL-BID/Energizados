...
> Capability: `error-handling` — coherent, public, backward-compatible exception hierarchy.
> Stable public API. Future changes require deprecation paths.
...
The hierarchy establishes types for each framework layer. Bare `ValueError`,
`RuntimeError`, and `TypeError` sites outside the defined exception types are
not automatically converted — they remain as internal preconditions where the
stdlib type is acceptable. Converting them is tracked separately when specific
sites require typed catch paths.

#### Scenario: unrelated bare raises are outside the typed hierarchy

- GIVEN bare `raise ValueError(...)` outside the fitted-guard or contract sites
- WHEN the code executes
- THEN the raise uses stdlib types, not framework exception types

### Requirement: Stability Commitment

The exception hierarchy is frozen public API. All 11 exception types listed
in the hierarchy table are stable. Future changes (renames, base-class changes,
removals) require a deprecation path — never a silent break. Adding new
subclasses of `EnergizadosError` is allowed and non-breaking.
