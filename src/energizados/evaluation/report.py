...
class ReportGenerator:
    ...
    def _build_html(
        ...
        has_shap = bool(
            plots_embedded and any(k.startswith("shap_") for k in plots_embedded)
        ) or bool(plots and any(k.startswith("shap_") for k in plots))
        ...
        shap_plot_keys = set()
        if has_shap:
            shap_section, shap_plot_keys = self._build_shap_section_html(
                metrics, plots_embedded=plots_embedded
            )
        ...
        <main class="main">
            ...
            <div class="section" id="section-visualizations">
                ...
                <div class="plots-grid">
                    {self._build_plots_html(plots_rel, plots_embedded=plots_embedded, plots_interactive=plots_interactive, extra_skip_keys=shap_plot_keys)}
                </div>
            </div>
...
"""
    ...
    def _build_shap_section_html(
        self,
        metrics: Dict,
        plots_embedded: Optional[Dict[str, str]] = None,
    ) -> tuple:
        """Builds dedicated SHAP explainability section.

        Returns:
            Tuple of (section_html, shap_plot_keys set)
        """
        shap_plot_keys = {"shap_summary", "shap_bar"}
        plots_embedded = plots_embedded or {}

        top_features = metrics.get("shap_top_features", [])
        top_features_html = ""
        if top_features:
            items = "".join(f"<li>{feat}</li>" for feat in top_features[:10])
            top_features_html = f"""
            <div style="margin-bottom:16px;">
                <h3 style="margin-bottom:8px;">Top Features by SHAP Importance</h3>
                <ol style="columns:2;column-gap:32px;">{items}</ol>
            </div>"""

        charts_html = ""
        for key in ["shap_summary", "shap_bar"]:
            if key in plots_embedded:
                label = "SHAP Summary Plot" if key == "shap_summary" else "SHAP Feature Importance"
                charts_html += f"""
                <div class="plot-container">
                    ...
                    <div class="plot-content">
                        <img src="{plots_embedded[key]}" alt="{label}">
                    </div>
                </div>"""

        if not top_features_html and not charts_html:
            return "", set()

        return (
            f"""
    <div class="section" id="section-shap">
...
""",
    ...
    def _build_plots_html(
        ...
        plots_embedded: Optional[Dict[str, str]] = None,
        plots_interactive: Optional[Dict[str, str]] = None,
        extra_skip_keys: Optional[set] = None,
    ) -> str:
        ...
        if extra_skip_keys:
            skip_keys |= extra_skip_keys

        # Human-readable labels for special plot keys
        _label_overrides = {
            "shap_summary": "SHAP Summary Plot",
            "shap_bar": "SHAP Feature Importance Bar Plot",
            "roc_curve": "ROC Curve",
        }
        ...
        for plot_name in all_keys:
            ...
            label = _label_overrides.get(plot_name, plot_name.replace("_", " ").title())
