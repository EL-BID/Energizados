...
class GeoCluster(BaseEstimator, TransformerMixin):
    """Assigns geographic cluster labels via KMeans on lat/lon coordinates.

    Fits a KMeans model on training coordinates and appends a ``geo_cluster``
    integer column on transform. Designed to be used as a global transformer
    in the preprocessing pipeline, typically BEFORE ``geo_features`` so that
    the cluster column is available for stratified time splits.
    ...
    def fit(self, X: pd.DataFrame, y=None) -> "GeoCluster":
        """Fit KMeans on valid lat/lon coordinates.
        ...
        lats = pd.to_numeric(X[self.lat_col], errors="coerce").values
        lons = pd.to_numeric(X[self.lon_col], errors="coerce").values
        ...
        coords_scaled = self.scaler_.fit_transform(coords)

        self.kmeans_ = KMeans(n_clusters=n_clusters, random_state=self.random_state, n_init=10)
        self.kmeans_.fit(coords_scaled)
        self.n_clusters_ = n_clusters

        logger.info(
            "GeoCluster: fitted %d clusters on %d valid coordinates", n_clusters, len(coords)
        )
        return self

    def transform(self, X: pd.DataFrame) -> pd.DataFrame:
        """Assign cluster labels and append ``geo_cluster`` column.
        ...
        Returns:
            pd.DataFrame: Original DataFrame with ``geo_cluster`` column appended.
        """
        df = X.copy()
        ...
        if valid_mask.any():
            coords = np.column_stack([lats[valid_mask], lons[valid_mask]])
            coords_scaled = self.scaler_.transform(coords)
            labels[valid_mask] = self.kmeans_.predict(coords_scaled)
