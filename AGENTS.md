...
training:
  ...
  split:
    ...
    # source_path accepts a literal path OR an "@etl_name" reference resolved at
    # run time against the executed ETLs (e.g. AssumedNegativesETL's output).
    # unlabeled_negatives:
    #   enabled: true
...
- `AssumedNegativesETL`: Derives **Assumed Negatives** (consumption rows whose client id has no inspection record, labeled `target=0` downstream by assumption) via an anti-join, and writes them to a standalone inspectable parquet. Input roles are positional and exactly 2 are required: `input: [consumption, inspections]` — each may be a literal path, an `@etl_name` reference, or a Hive-style partitioned parquet directory (e.g. an incremental ETL's output; the discovered `*.parquet` files are read directly, without materializing partition keys as columns; a directory with no parquet files raises `ETLError` naming the role). `custom_class: "energizados.etl.pipeline.AssumedNegativesETL"`. Params: `id_column` (required — join column present in both sources; `ValueError` at instantiation when missing, `ETLError` naming role+column+available columns at runtime). Never modifies either input; duplicate consumption ids are emitted as-is and counted; logs join stats (rows in / matched / emitted); an empty anti-join still writes a 0-row parquet with a WARNING. Reference the output from training as `split.unlabeled_negatives.source_path: "@negatives"` (ETL-reference mode: train schema wins — extra source columns are dropped, missing train columns are NaN-filled; `split_metadata.json` reports `unlabeled_negatives_source_mode: "etl_ref"` and `unlabeled_negatives_labels_assumed: true` whenever injection is enabled, including 0 rows).
