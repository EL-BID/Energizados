...
- **LightGBM: configurable `n_jobs` with framework default `-1`.** The LightGBM classifier is now built with `n_jobs=-1` (all cores) by default, and `n_jobs` (or any other key outside the search space) specified in a model's `hyperparams` is honored — including when `hyperparam_search` is enabled, which previously replaced user hyperparams with the search's `best_params_` and silently dropped them. During hyperparameter search each worker now fits its pipeline clone with `n_jobs=1` to avoid oversubscription (the search itself parallelizes with `n_jobs=-1`); the final fit applies the user/default `n_jobs`. Tests: `tests/test_lgbm_model.py`.

- **Hyperparameter search: configurable `n_jobs` (`hyperparam_search.n_jobs`).** Exposes the `RandomizedSearchCV` parallelism as a YAML key for lightgbm, catboost and xgboost (constructor kwarg `search_n_jobs`). Defaults preserve previous behavior per model: `-1` (lightgbm), `4` (catboost), `-1` (xgboost). Complements the model-level `hyperparams.n_jobs` so users can cap total cores used during train — each search worker fits its model clone sequentially. Tests: `tests/test_search_n_jobs.py`.
...
- **Docs: step-by-step Spanish tutorial overhauled for VPS/proxy workflows** (`docs/tutorials/step-by-step_es.md`). Restructured into 16 numbered steps (clone → train → inference), adding HTTP/HTTPS proxy configuration for VPS-first environments, SSH/tmux access with artifact-download tunnels, optional geographic features through proxy, inference and inference-analysis steps, a SLES 15 / openSUSE Leap 15+ prerequisites table that calls out manylinux/duckdb/pyarrow wheel compatibility, and a "Problemas frecuentes" troubleshooting section. Registered in `mkdocs.yml` nav.
...
- **Documentation: 3 of 8 base classes reclassified from "frozen public API" to "extension points".** `AGENTS.md` previously claimed all 8 base classes were "frozen public API". By the deep-module test (*one adapter means a hypothetical seam; two adapters means a real one*), 3 of them — `BasePipeline` (0 framework consumers), `BaseEvaluator` (1 adapter), `BaseFeatureEngineering` (1 adapter) — do not have a second consumer to validate their interface shape, so calling them "frozen" tied the framework's hands against future refactors without earning real extensibility. They are now described as **extension points for users**: still supported, but not stability-frozen, and will be promoted to frozen API when a second adapter appears in the wild. The other 5 (`BaseModel`, `BaseETL`, `BaseFeatureSelector`, `BaseExplorer`, `BaseInference`) keep the "frozen" claim, now backed by an adapter-count table in `AGENTS.md`.
...
- **Removed: dead `_is_noop_load` flag on `BaseETL`.** The flag (class attribute + instance attribute + a `run()` branch consuming it) was a three-layer mechanism defended by no consumer: `CleanFilesETL` — the only ETL that set it to `True` — overrides `run()` directly and never calls `super().run()`, so the branch was unreachable in practice. The `noop_load()` method is kept as a hook for ETLs that override `run()` directly. No behavior change; just removes the dead mechanism and the three tests that pinned it.
- **Security: web console refuses to bind to non-loopback interfaces without explicit opt-in.** The web console has no authentication layer and accepts arbitrary YAML as job configuration in a context where the worker calls `register_allowed_prefix("src")` and `os.chdir(job_dir)`, so binding to anything other than `127.0.0.1` exposes a code-execution surface. The launcher now fails-closed with a loud stderr warning and exit code 2 if `--host` is set to a non-loopback value without the new `--allow-remote` flag. Loopback hosts (`127.0.0.1`, `localhost`, `::1`) keep working unchanged. The full threat model is documented at the top of `docs/web-console/DEPLOYMENT.md`.
  - **Migration:** if you were binding with `--host 0.0.0.0` (or any non-loopback IP), add `--allow-remote` to acknowledge the tradeoff. Default `127.0.0.1` users see no change.
- **Fixed: `BorutaSelector` shadow features are now deterministic across identical runs.** The shadow column used in each Boruta iteration was generated with the unseeded global NumPy RNG, so `selected_features_` could vary across two runs with identical inputs and the same `random_state`. The shadow is now derived from `np.random.default_rng(self.random_state + i)`, mirroring the per-iteration seed already used by `BorutaPy`. Reproducible feature selection matters for audit and model governance.
- **Fixed: ADRs consolidated under `docs/adr/` and surfaced in the docs nav; PyPI URLs point to the correct repository.** The four ADRs were scattered across `src/energizados/docs/adr/` and `src/energizados/web/docs/adr/` with no entry in the mkdocs nav, making the framework's most important architectural decisions invisible on the published docs site. They are now under `docs/adr/` with coherent numbering (0001..0004) and reachable via Advanced > Architecture Decision Records. PyPI `[project.urls]` was pointing at the non-existent `github.com/energizados/energizados` and `energizados.readthedocs.io`; now points at `github.com/EL-BID/Energizados` (Documentation → README, since Pages is not currently published).
...
- **GeoFeatures: transformer owns geographic clustering (ADR-0001)** — KMeans clustering (`geo_cluster`) and the train/infer `geo_model.pkl` hand-off move from `GeoFeaturesETL` (file-I/O layer) into the `GeoFeatures` transformer (`preprocessing/`), matching `CONTEXT.md`'s definition of a Transformer. `GeoFeatures` now exposes pure scikit-learn semantics (`fit`/`transform`/`save`/`load`) plus `include_cluster` (default `false`, preserving existing `global_transformers` usage). `GeoFeaturesETL` becomes a thin wrapper that delegates to the transformer while keeping its full param surface and `ETLError` contract. Decision recorded in `docs/adr/0001-geo-features-as-transformer.md` (first framework-core ADR). No public-API changes; internal refactor only.
