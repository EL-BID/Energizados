...
- **Config: `output_name` — run-directory name from config (mirrors CLI `-n`)** — Every pipeline section (`train`/`infer`/`eda`/`etl`) now accepts an optional top-level `output_name` string that sets the run-directory NAME, exactly like the CLI `--name`/`-n` flag. `PipelineDirector._resolve_output_name` resolves it with the same priority as `output_base_dir` (`train > infer > eda > etl`; default `None` → timestamped run dir). Precedence: the CLI `-n` wins over the config value (explicit override). `build()` now passes `run_name = self._run_name or self._resolve_output_name()` to `generate_run_dir`, so the existing path-traversal guard still applies. Declared in `ETL_SCHEMA`/`TRAINING_SCHEMA`/`INFER_SCHEMA` (parallel to `output_base_dir`) so jsonschema accepts it, and added to `ETLOrchestrator._EXCLUDED_TOP_KEYS` so it is not registered as a bogus DAG node. Documented in the `train.yaml`/`infer.yaml` templates. Fully backward compatible: absent `output_name` keeps the timestamped default.
- **Inference: `sort_by_probability` — output sorted by probability descending by default** — `infer:` config now accepts `sort_by_probability` (boolean, default `true`). When true (the default), the predictions output is sorted by `probability` DESCENDING (most suspicious clients first), applied AFTER `output_columns` selection on the final frame so input columns reorder with their probability. Set `sort_by_probability: false` to preserve input order. Declared in `INFER_SCHEMA`; implemented in `InferenceStep._save_output`. The default change is intentional (requested operational behavior for fraud scoring); the one existing test that asserted input-order prediction values was isolated with `sort_by_probability: false` so it still verifies column-selection behavior. New `tests/test_inference/test_inference_sort.py` locks default-on, disabled, and no-output_columns cases.
...
- **Inference: `output_columns` is now the self-sufficient output selector; `output_include_input` deprecated** — `output_columns` selects and orders the FINAL output columns over the combined `[input + prediction + probability + rule_*]` frame, applied last, and is now SELF-SUFFICIENT: input columns named in it are included automatically (no `output_include_input` needed). This enables two workflows in one place: (a) include only a subset of input columns (list them; unlisted are dropped — previously `output_include_input` was all-or-nothing), and (b) drop the `prediction` column by simply omitting it from the list. Previously `output_columns` filtered only the prediction-side columns *before* the input was concatenated, so input columns named in it were silently ignored (doc/code mismatch, now fixed). `output_include_input` is now DEPRECATED: `output_include_input: true` (without `output_columns`) still prepends all input columns but emits a `DeprecationWarning` pointing to `output_columns`; when both are set, `output_columns` wins and `output_include_input` is ignored (with a warning). If `output_columns` is absent and `output_include_input` is false/absent, output defaults to `[prediction, probability]` (+ any `rule_*`) — fully backward compatible. Unknown/unavailable columns are warned and skipped rather than crashing. The selected columns are recorded in the `.metadata.json` sidecar. The schema description and `infer.yaml` template document the new semantics. Also fixed a latent `NameError` (unbound `_model_path`) in the metadata sidecar call under hierarchical inference.
- **Dependencies: `geobr` declared in `pyproject.toml`** — `preprocessing/geo_features.py` (and `GeoFeaturesETL`) import `geobr` lazily to read IBGE shapefiles, but the package was not declared anywhere, so a clean `pip install energizados` could not use geo features. Added `geobr>=0.2.0` to `dependencies` and `geobr.*` to mypy `ignore_missing_imports`. Note: `geobr` pulls the GeoPandas/Shapely/GDAL stack, increasing the install footprint.

### Changed

- **Inference: default output now includes ALL columns (behavior change)** — When `output_columns` is not set, the predictions file now contains ALL columns (original input + `prediction` + `probability` + any `rule_*`) instead of only `[prediction, probability]`. This supersedes the prior minimal default documented above. `output_include_input` is now a fully redundant no-op (kept for backwards compatibility; still emits a `DeprecationWarning` when explicitly set) — use `output_columns` to select a subset. **Migration:** inference configs that relied on the minimal 2-column default must now set `output_columns: [prediction, probability]` to restore it.
- **Inference: `index.html` no longer regenerated for inference runs** — `RunManager.generate_index_html` is now skipped for non-training run types (`inference`/`eda`/`etl`), because the index lists training evaluation reports that those runs don't produce. Previously the index was rewritten after every run; now only training runs (and training-priority merged runs) update it. `generate_index_html` now returns `Optional[Path]` (None when skipped); `ConfigPipelineBuilder.generate_index_html` propagates it, and the CLI prints "Index updated" only when a file was actually written.
- **Inference: logs the generated output columns** — `InferenceStep._save_output` now logs the final column list and count (`Predictions output columns (N): [...]`) for every predictions file written, alongside the existing "Predictions saved to" line.
- **Inference: `output_path` renamed to `output_predictions_path` (deprecated alias kept)** — The predictions output file is now configured via `output_predictions_path`. The legacy `output_path` remains as a deprecated alias: when it is the only key set it still works but emits a `DeprecationWarning`; when both are set `output_predictions_path` wins and `output_path` is ignored (with a warning). Declared in `INFERENCE_SCHEMA`, resolved in `InferenceBuilder.build`/`execute`, and checked by `cli validate`. Internal context key `inference_output_path` (run metadata) is unchanged. Templates and docs (`infer.yaml.tpl`, `README.md.tpl`, `docs/.../infer.md`, tutorials) updated to the canonical key.
- **Inference: `segment_thresholds.fallback_threshold` removed; fallback is the global `threshold`** — The fallback threshold for unknown segment values was already the global `threshold` whenever `fallback_threshold` was omitted; the redundant `fallback_threshold` key is now removed from the schema and code. The global `threshold` is the single source of the fallback. Migration is warn+ignore: a config still carrying `fallback_threshold` emits a `WARNING` at predict time and the value is ignored (no crash). The internal `apply_segment_thresholds(...)` helper signature is unchanged. The `segment_thresholds` schema now documents this in its description.

### Fixed

- **Inference: `output_include_input` no longer pads filtered-out rows with NaN** — `InferenceStep` captured `original_data = data.copy()` *before* applying `columns_filter`, then `_save_output` did `pd.concat([original_data, predictions_df], axis=1)` (an outer join). When `columns_filter` removed rows, `original_data` still held the full pre-filter set while `predictions` had only the filtered count, so the enriched CSV gained one NaN-padded `prediction`/`probability` row per filtered-out input row (e.g. a 3.4M-row output with only 804k actual predictions, the rest empty `,,`). The capture now happens *after* `columns_filter`, as an unconditional sibling of the filter block, so the enriched input is always aligned 1:1 with the predicted rows. Affects only `output_include_input: true` runs; the minimal 2-column output (`prediction`, `probability`) was never affected. Regression test `test_columns_filter_with_include_input_no_padding` added.
- **Tests: `_make_mock_model` returns exactly `len(X)` probabilities** — The inference test helper's `_predict_proba` used `_proba * (n // len(_proba) + 1)` when `len(_proba) < n`, which over-returned whenever `len(_proba)` did not divide `n` (e.g. `[0.5, 0.5]` for `n=4` produced 6 values). That silently inflated prediction arrays and masked the `output_include_input` padding bug during the initial RED. Replaced with a cycling `[ _proba[i % len(_proba)] for i in range(n) ]` that always returns exactly `n` values, plus an empty-input guard. New `TestMockModelHelper` locks the contract (exact length, cycling, empty input).
